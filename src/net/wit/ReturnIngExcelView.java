 ...
 */
public class ReturnIngExcelView extends AbstractExcelView {
	...
	public ReturnIngExcelView(String filename, Collection<net.wit.entity.Order> data) {
		...
		while (iterator.hasNext()) {
			...
			for (int i = 0; i < orderSize; i++) {
				...
				if(i==orderSize-1&&orderSize>1){
					...
					ExcelUtil.setCellStyle(workbook, sheet, startCount,count,9,18);
				}
			...
			rowCell18.setCellValue(order.getApplayReturns() != null ? DateUtil.changeDateToStr(order.getApplayReturns().getCreateDate(), DateUtil.CN_DISPLAY_DATE_FULL) : "");
			rowCell19.setCellValue(order.getMemo());
			count++;
		}
		// 自适应宽度
		for (int i = 0; i <= 19; i++) {
