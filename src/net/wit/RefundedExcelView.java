 ...
 */
public class RefundedExcelView extends AbstractExcelView {
	...
	public RefundedExcelView(String filename, Collection<net.wit.entity.Order> data) {
	...
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		while (iterator.hasNext()) {
			...
			rowCell18.setCellValue(order.getCompleteRefunds() != null ? DateUtil.changeDateToStr(order.getCompleteRefunds().getCreateDate(), DateUtil.CN_DISPLAY_DATE_FULL) : "");
			rowCell19.setCellValue("已退款");
			rowCell20.setCellValue(order.getCompleteRefunds() != null ? DateUtil.changeDateToStr(order.getCompleteRefunds().getModifyDate(), DateUtil.CN_DISPLAY_DATE_FULL) : "");
			rowCell21.setCellValue(order.getMemo());
			...
			for (int i = 0; i < orderSize; i++) {
				...
				if(i==orderSize-1&&orderSize>1){
					...
					ExcelUtil.setCellStyle(workbook, sheet, startCount,count,9,20);
				}
