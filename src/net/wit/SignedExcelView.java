 ...
 */
public class SignedExcelView extends AbstractExcelView {
	...
	public SignedExcelView(String filename, Collection<net.wit.entity.Order> data) {
	...
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		headercell3.setCellValue("店长");
		HSSFCell headercell4 = header.createCell(4);
		headercell4.setCellValue("店长推荐人");
		...
		headercell10.setCellValue("分享佣金");
		...
		while (iterator.hasNext()) {
			...
			HSSFCell rowCell18 = rowItem.createCell(18);
			HSSFCell rowCell19 = rowItem.createCell(19);
			HSSFCell rowCell20 = rowItem.createCell(20);
			HSSFCell rowCell21 = rowItem.createCell(21);
			HSSFCell rowCell22 = rowItem.createCell(22);
			
			rowCell0.setCellValue(order.getSn());
			rowCell1.setCellValue("已签收");
			rowCell2.setCellValue(order.getMember().getMobile());
			rowCell3.setCellValue(order.getOwner().getMobile());
			if(order.getShopkeeper().getRecommendMember()!=null){
			...
			rowCell17.setCellValue(order.getEffect() == null ? "" : DateUtil.changeDateToStr(order.getEffect().getCreateDate(), DateUtil.CN_DISPLAY_DATE_FULL));
			rowCell18.setCellValue(order.getShippingMethodName());
			rowCell19.setCellValue(order.getDefaultShipping() != null ? order.getDefaultShipping().getDeliveryCorp() : "");
			rowCell20.setCellValue(order.getDefaultShipping() != null ? order.getDefaultShipping().getTrackingNo() : "");
			rowCell21.setCellValue(DateUtil.changeDateToStr(order.getModifyDate(), DateUtil.CN_DISPLAY_DATE_FULL));
			rowCell22.setCellValue(order.getMemo());
			...
			for (int i = 0; i < orderSize; i++) {
				...
				if(i==orderSize-1&&orderSize>1){
					...
					ExcelUtil.setCellStyle(workbook, sheet, startCount,count,9,21);
				}
