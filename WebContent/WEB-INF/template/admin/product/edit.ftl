...
$().ready(function() {
	...
	$specificationIds.click(function() {
		...
		if ($this.prop("checked")) {
			...
			$specificationProductTable.find("td[class^=stock_]").show();
			$specificationProductTable.find("td[class^=price_]" ).show();
			$specificationProductTable.find("td[class^=rent_]").show();
			$specificationProductTable.find("td[class^=cost_]").show();
		} else {
	...
	$inputForm.validate({
		...
		submitHandler: function(form) {
			...
			if ($specificationIds.filter(":checked").size() > 0 && $specificationProductTable.find("tr:gt(1)").size() == 0) {
				...
				var $specificationId=$($specificationIds.filter(":checked")[$specificationIds.filter(":checked").length-1]);
				$specificationProductTable.find("input[name^=stock_]").attr("name","stock_"+$specificationId.val());
				$specificationProductTable.find("input[name^=price_]").attr("name","price_"+$specificationId.val());
				$specificationProductTable.find("input[name^=rent_]").attr("name","rent_"+$specificationId.val());
				$specificationProductTable.find("input[name^=cost_]").attr("name","cost_"+$specificationId.val());
	...
	$culc.click(function(){
		...
		$rentss.each(function(){
			...
			if(price!=""&&(/^[0-9]+.?[0-9]*$/.test(price))){
				_this.val(parseFloat(price)*parseFloat($rentPen.val())/100);
			}
		});
	});
});
				...
				<td>
					<table id="specificationProductTable" class="input">
						...
						<tr class="hidden">
							...
							[/#list]
							[#list specifications as specification]
							[#if tr && product.specifications?seq_contains(specification)]
							...
							<td class="cost_${specification.id} [#if !product.specifications?seq_contains(specification)] hidden[/#if]">
								<input type="text" name="cost_${specification.id}" value="${product.cost}" />
							</td>
							[#assign tr=false /]
								[/#if]
							[/#list]
						...
						[#list product.siblings as specificationProduct]
							<tr>
								...
								[/#list]	
								[#list specifications as specification]	
							[#if tr && specificationProduct.specifications?seq_contains(specification)]
							...
							<td class="cost_${specification.id} [#if !specificationProduct.specifications?seq_contains(specification)] hidden[/#if]">
								<input type="text" name="cost_${specification.id}" value="${specificationProduct.cost}"/>
							</td>
								[#assign tr=false /]
								[/#if]
								[/#list]
